...
export default function AdminImagesClient({ initialWhiskies }: AdminImagesClientProps) {
  ...
  const openFileSelector = (whiskyId: string, whiskyName: string) => {
    const input = document.createElement('input')
    input.type = 'file'
    input.accept = 'image/*'
    input.onchange = (e) => handleFileUpload(whiskyId, whiskyName, e as any)
    input.click()
  }
              ...
              return (
                <div key={whisky.id} className="border border-gray-200 rounded-lg p-4">
                  ...
                  {result?.success ? (
                    ...
                    <button
                      onClick={() => openFileSelector(whisky.id, whisky.name)}
                      disabled={uploading}
