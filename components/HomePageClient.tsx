    ...
    <LikesProvider userId={user?.id} initialLikedIds={initialLikedIds}>
      ...
      <div className="flex">
        ...
        <div className="flex-1">
          {/* 헤더 - 모바일 최적화 */}
          <header className="flex items-center justify-between mb-4 sm:mb-8 h-14 sm:h-auto">
            {/* 로고 & 사이트명 */}
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <h1 className="text-xl sm:text-4xl font-bold text-amber-800 font-[family-name:var(--font-jolly-lodger)]">Maltlog</h1>
            </div>

            {/* 태그라인 - 모바일에서 숨김 */}
            <span className="hidden sm:block text-sm sm:text-base text-gray-500 absolute left-1/2 transform -translate-x-1/2">몰트로그, 위스키의 모든 기록</span>
                ...
                <div data-mobile-menu className="absolute top-full right-0 mt-1 bg-white border border-gray-200 shadow-lg rounded-lg overflow-hidden z-50 w-40">
                  <div className="py-1">
                    <button
                      onClick={() => {
                        setIsMobileMenuOpen(false)
                        // HOME은 현재 페이지이므로 아무 동작 없음
                      }}
                      className="w-full text-left px-3 py-2 text-sm font-medium text-red-500 bg-gray-50"
                    >
                      HOME
                    </button>
                    ...
                    {user ? (
                      <>
                        ...
                        <button
                          ...
                          className="w-full text-left px-3 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors"
                        >
                          로그아웃
                        </button>
                      </>
                      ...
                      <>
                        ...
                        <button
                          onClick={() => {
                            setIsMobileMenuOpen(false)
                            navigateWithTransition('/login', '로그인 페이지로 이동 중...')
                          }}
                          className="w-full text-left px-3 py-2 text-sm font-medium text-amber-800 hover:bg-amber-50 transition-colors"
                        >
                          로그인
                        </button>
                      </>
          ...
          {/* 검색창 - Sticky 위치 */}
          <div className="sticky top-0 z-30 bg-rose-50 py-3 mb-4 sm:mb-6 border-b border-rose-100">
            <div className="relative max-w-md mx-auto">
              <input
                ...
                className="w-full h-11 px-4 pr-10 text-sm bg-white border border-rose-300 rounded-lg shadow-sm focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-200 placeholder-gray-400 text-gray-700 transition-all duration-200"
              />
              {searchQuery.trim() && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-rose-600 text-lg transition-colors"
                >
                  ✕
                </button>
              )}
          ...
          <section className="mb-8 sm:mb-12">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6">
              <div className="text-center sm:text-left mb-3 sm:mb-0">
                {!searchQuery.trim() && !showAllWhiskies && (
                  <h2 className="text-lg sm:text-xl font-bold text-gray-800">위스키 컬렉션</h2>
                )}
                {!searchQuery.trim() && showAllWhiskies && (
                  <h2 className="text-lg sm:text-xl font-bold text-gray-800">모든 위스키</h2>
                )}
                {searchQuery.trim() && (
              ...
              <div className="flex justify-center sm:justify-end gap-2">
                {!searchQuery.trim() && (
                  <button
                    ...
                    className="text-xs sm:text-sm font-medium text-white bg-amber-900 hover:bg-amber-800 px-3 py-1.5 sm:py-2 rounded-full disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
                ...
                {searchQuery.trim() && (
                  <button
                    onClick={() => setSearchQuery('')}
                    className="text-xs sm:text-sm font-medium text-rose-700 hover:text-rose-800 transition-colors bg-rose-50 hover:bg-rose-100 px-3 py-1.5 sm:py-2 rounded-full"
                  >
                    검색 지우기
                  </button>
                )}
...
function WhiskyCard({ whisky, navigateWithTransition }: { whisky: WhiskyData, router: unknown, navigateWithTransition: (path: string, message: string) => void }) {
  ...
  return (
    <div
      className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 cursor-pointer relative"
      onClick={handleClick}
    >
      {/* 위스키 이미지 영역 - 정사각형 비율 */}
      <div className="aspect-square bg-gray-100 flex items-center justify-center relative">
        ...
        <button
          ...
          className={`absolute top-2 right-2 flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium transition-all duration-200 ${
            !user
              ? 'bg-white/80 text-gray-400 cursor-not-allowed'
              : isWhiskyLiked
              ? 'bg-white/90 text-red-500 shadow-sm'
              : isLikeHovered
              ? 'bg-white/90 text-red-400 shadow-sm'
        ...
        >
          <span className={`text-sm transition-all duration-200 ${isWhiskyLiked ? 'scale-110' : ''}`}>
            {isWhiskyLiked ? '♥' : '♡'}
          </span>
          <span className="font-semibold">{formatLikeCount(currentLikes || 0)}</span>
        </button>
      </div>
      ...
      <div className="p-2">
        {/* 위스키 이름 - 말줄임 처리 */}
        <div className="text-xs sm:text-sm font-semibold mb-1 text-gray-800 leading-tight truncate" title={whisky.name}>
          {whisky.name}
        </div>
            ...
            <div className="flex items-center gap-0.5">
              {[1, 2, 3, 4, 5].map((starIndex) => {
                ...
                if (starIndex <= fullStars) {
                  // 채워진 노란 별
                  return (
                    <span key={starIndex} className="text-yellow-500 text-xs">
                      ★
                    </span>
                  );
                  ...
                  // 반별: 약간 어두운 노란색
                  return (
                    <span key={starIndex} className="text-yellow-400 text-xs">
                      ★
                    </span>
                  );
                } else {
                  // 빈 별 (투명)
                  return (
                    <span key={starIndex} className="text-gray-300 text-xs">
                      ☆
                    </span>
                  );
                }
